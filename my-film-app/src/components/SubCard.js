...
const SubCard = ({silver, price , text , BPrice , otherContainerClassNames,mounth,detail }) => {
  return (
    <div className={`dark:bg-slate-900 bg-white relative flex justify-between items-center border border-primary rounded-md px-4 py-3 ${otherContainerClassNames}`}>
      <span className='-mt-20 p-1 text-sm dark:text-slate-300 text-gray-700 rounded-2xl dark:bg-slate-700 bg-gray-200 absolute'>{detail}</span>
